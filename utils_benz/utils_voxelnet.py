...
def cal_anchors():
    ...
    anchors = np.stack([cx, cy, h, w, l, r], axis=-1)  ## cz is ignore

    return anchors


##TODO: the anchors are the main different,
def cal_rpn_target(labels, anchors, cls='Car', coordinate='lidar'):
    # Input:
    #   labels: (N, N')
    #   feature_map_shape: (num_rpn_points)
    #   anchors: (batchsize, num_rpn_points, 2, 6) #order: [cx, cy, h, w, l, r]
    # Output:
    #   pos_equal_one (N, num_rpn_points, 2)
    ...
    for batch_id in range(batch_size):
        # BOTTLENECK
        anchors_reshaped = anchors[batch_id,...]
        anchors_d   =   anchors_all_d[batch_id,...]
        anchors_standup_2d = anchor_to_standup_box2d(
            anchors_reshaped[:, [0, 1, 3, 4]])   ## take default: 0 and pi/2, it is hard-coding style
        # BOTTLENECK
        gt_standup_2d = corner_to_standup_box2d(center_to_corner_box2d(
            batch_gt_boxes3d[batch_id][:, [0, 1, 3, 4, 5]], coordinate=coordinate))
        ...
        index_rpn, index_anchor = np.unravel_index(
            id_pos, (num_rpn_points, num_anchors))
        ...
        ##  no z direction
        #targets[batch_id, index_rpn,  np.array(index_anchor) * num_regression + 2] = (
        #    batch_gt_boxes3d[batch_id][id_pos_gt, 2] - anchors_reshaped[id_pos, 2]) / cfg.ANCHOR_H
        targets[batch_id, index_rpn, np.array(index_anchor) * num_regression + 2] = np.log(
            batch_gt_boxes3d[batch_id][id_pos_gt, 3] / anchors_reshaped[id_pos, 2])
        targets[batch_id, index_rpn, np.array(index_anchor) * num_regression + 3] = np.log(
            batch_gt_boxes3d[batch_id][id_pos_gt, 4] / anchors_reshaped[id_pos, 3])
        targets[batch_id, index_rpn, np.array(index_anchor) * num_regression + 4] = np.log(
            batch_gt_boxes3d[batch_id][id_pos_gt, 5] / anchors_reshaped[id_pos, 4])
        targets[batch_id, index_rpn,  np.array(index_anchor) * num_regression + 5] = (
            batch_gt_boxes3d[batch_id][id_pos_gt, 6] - anchors_reshaped[id_pos, 5])

        index_rpn, index_anchor = np.unravel_index(
            id_neg, (num_rpn_points, num_anchors))
        neg_equal_one[batch_id, index_rpn, index_anchor] = 1
        # to avoid a box be pos/neg in the same time
        index_rpn, index_anchor = np.unravel_index(
            id_highest, (num_rpn_points, num_anchors))
        neg_equal_one[batch_id, index_rpn, index_anchor] = 0
