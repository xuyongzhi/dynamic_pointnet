...
def region_proposal_loss(pred_class, pred_box, gt_box, smpw, xyz):
    ...
    output_pred_box_one_batch   = np.zeros((NUM_BATCH, N*A, NUM_regression), dtype = np.float32)
    output_box_targets          = np.zeros((NUM_BATCH, N*A, NUM_regression), dtype = np.float32)
    output_box_inside_weights   = np.zeros((NUM_BATCH, N*A, NUM_regression), dtype = np.float32)
    output_box_outside_weights  = np.zeros((NUM_BATCH, N*A, NUM_regression), dtype = np.float32)
    output_pred_class_one_batch = np.zeros((NUM_BATCH, cfg.TRAIN.RPN_BATCHSIZE, CLASS), dtype = np.float32)
    output_labels               = np.zeros((NUM_BATCH, cfg.TRAIN.RPN_BATCHSIZE), dtype = np.float32)
...
def _smooth_l1(sigma ,box_pred, box_targets, box_inside_weights, box_outside_weights):
    ...
    """
    sigma2 = sigma * sigma
    inside_mul = tf.multiply(box_inside_weights, tf.subtract(box_pred, box_targets))
    ...
    loss_reg = tf.reduce_mean(tf.reduce_sum(outside_mul, axis=1))
