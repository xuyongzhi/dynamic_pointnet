...
def placeholder_inputs(batch_size, block_sample,data_num_ele,label_num_ele, sg_bidxmaps_shape, num_regression):  ## benz_m
    ...
    with tf.variable_scope("pls") as pl_sc:
        ...
        gt_box_pl = tf.placeholder(tf.float32, shape=(batch_size, None, num_regression+1))   # benz_m, label = category, l, w ,h, alpha, x, y, z, +1 is for the category
        ...
        # flatten_bidxmaps_pl = tf.placeholder(tf.int32,shape=
        # (batch_size,)+flatten_bidxmaps_shape[0:-1]+(2,),name="flatten_bidxmaps_pl")
        # #benz_m
        ...
        return pointclouds_pl, gt_box_pl, sg_bidxmaps_pl, sgf_config_pls    ## benz_m
...
def get_model(modelf_nein, rawdata, is_training, num_class, sg_bidxmaps, sgf_configs, sgf_config_pls, bn_decay=None,IsDebug = False):     ## benz_m
    """
...
def get_loss(batch_size, pred_class, pred_box, gt_box, xyz):
    '''
