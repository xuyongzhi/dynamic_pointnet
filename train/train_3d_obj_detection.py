...
os.system('cp %s/models/pointnet_3d_obj_detection.py %s' % (ROOT_DIR,LOG_DIR)) # bkp of model def
os.system('cp %s/train/train_3d_object_detection.py %s' % (ROOT_DIR,LOG_DIR)) # bkp of train procedure
os.system('cp %s/utils_xyz/configs.py %s' % (ROOT_DIR,LOG_DIR)) # bkp of train procedure
os.system('cp %s/utils_xyz/pointnet_blockid_sg_util.py %s' % (ROOT_DIR,LOG_DIR)) # bkp of train procedure
...
NUM_CHANNELS = cfg.NUM_CHANNELS  # x, y, z
num_classes =  cfg.NUM_CLASSES   # bg(car), fg
num_regression = cfg.NUM_REGRESSION
...
def train_eval(train_feed_buf_q,eval_feed_buf_q):
    with tf.Graph().as_default():
        ...
        for epoch in range(epoch_start,epoch_start+MAX_EPOCH):
            ...
            if epoch%10 == 0 and EVAL_IN_TRAIN:
...
def train_one_epoch(sess, ops, train_writer,epoch,train_feed_buf_q,pctx,opts, sg_bm_extract_idx):
    ...
    while (batch_idx < num_batches-1) or (num_batches==None):
        ...
        if ISDEBUG  and  epoch == 0 and batch_idx ==5:
                ...
                summary,  step, _, all_loss,   classification_loss,    output_regression_loss,  output_classification_positive_loss, output_classification_negative_loss \
...
def eval_one_epoch(sess, ops, test_writer, epoch,eval_feed_buf_q):
    ...
    t_batch_ls = []
    all_gt_box = []
    all_pred_class_feature_val = []
    all_pred_box_feature_val = []
    all_xyz   = []
    batch_idx = -1
    # label
    while (batch_idx < num_batches-1) or (num_batches==None):
        ...
        all_pred_class_feature_val.append(pred_prob_val)  # the all_pred_class_val is the list, num_batches x BATCH_SIZE x point_num x 4, all_pred_val[n] is the narray of BATCH_SIZE x point_num
        all_pred_box_feature_val.append(pred_box_feature_val)  # the all_pred_box_val is also list, num_batches x BATCH_SIZE x point_num x 14, all_pred_box_val[n] is the narray of BATCH_SIZE x point_num
    ...
    # format of all_pred_box_featurees: l, w, h, theta, x, y, z, score
    # format of gt_boxes: type, l, w, h, theta, x, y, z
    # put assemble all_pred_class_feature_val and all_pred_box_feature_val together accroding to
    # the format of all_pred_box_featurees
    # using 0.05 to select the all prediction, getting all_3D_box
    # using nms_3d to filter out the all bounding box
    print('---------------------------')
    print('Start evaluation!!')
    all_pred_box_featurees, all_gt_boxes =  boxes_assemble_filter(all_pred_class_feature_val, all_pred_box_feature_val, all_xyz, all_gt_box, 0.05)

    # caculate the average precision with the detection results
    aveg_precision = evaluation_2d(all_pred_box_featurees, all_gt_boxes, cfg.TEST.RPN_THRESH)
    # delete the all_gt_box, all_pred_class_feature_val and all_pred_box_feature_val to save
    # memory
...
def boxes_assemble_filter(all_pred_class_feature_val, all_pred_box_feature_val, all_xyz, all_gt_box , thresh = 0.05):

    #all_pred_box_featurees = np.zeros([1,8])  #l, w, h, theta, x, y, z, score
    all_pred_box_featurees = []  # saved in list
    num_batch = len(all_pred_class_feature_val)
    batch_size = all_pred_class_feature_val[0].shape[0]
    gt_box_ = []
    num_anchors = cfg.NUM_ANCHORS
    ...
    for i in range(num_batch):
        for j in range(batch_size):
            ...
            temp_pred_class_feature = np.array([all_pred_class_val[i][j,:,(x*num_class+1):((x+1)*num_class)] for x in range(num_anchors)]).transpose(1, 0, 2) ##shape: 512 x num_anchors x 1
            temp_pred_class_feature = temp_pred_class.reshape(-1, 1)  # shape: n x 1

            temp_all_box = bbox_transform_inv_2d(all_pred_box_feature_val[i][j,:,:], all_xyz[i][j,:,:])  # key function

            temp_all_       =  np.concatenate(( temp_all_box,temp_pred_class_feature), axis=1)
            ...
            if temp_all_.shape[0] > 0:  ## there is no prediction box whose prediction is over thresh
                ...
                all_pred_box_featurees.append(temp_all_)
            gt_box_.append(all_gt_box[i][j])

    return all_pred_box_featurees, gt_box_
