...
def zip_extract(house_dir,groupe_name,house_name,file_name,file_format,zipf):
    ...
    '''
    file_path = house_dir + '/%s/%s/%s.%s'%(house_name,groupe_name,file_name,file_format)
...
class Matterport3D_Prepare():
    ...
    def Parse_house_regions(self):
        ...
        #print('region num = %d'%(num_region))
        for k in range(num_region):
            file_name = 'region'+str(k)
            ply_path = zip_extract(self.house_dir,'region_segmentations',self.house_name,file_name,'ply',rs_zf)
            self.Parse_Region_Ply(ply_path,k)
