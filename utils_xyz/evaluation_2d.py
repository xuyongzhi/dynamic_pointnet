...
from nms_2d import caculate_2d_overlap
...
def evaluation_2d( all_pred_boxes, all_gt_boxes, threshold = 0.5 ):
    '''
    ...
    '''
    # change format of all_pred_boxes into type: index,l, w, theta, x, y, score
    num_all_boxes = len(all_pred_boxes)
    np_all_boxes = np.zeros([1 ,7])
    ...
    sorted_ind    = np.argsort(-np_all_boxes[:,6])
    sorted_scores = np.sort(-np_all_boxes[:,6])
    ...
    for j in range(num_all_boxes):
       ...
       all_gt_boxes_coordinate.append(convert_to_list_points(all_gt_boxes[j][:,1:6]))
    ...
    for d in range(num_ind):   # index in sorted_ind
        ...
        coordinate_cur = convert_to_list_points(np_all_boxes[ np_index_cur, 1:6].reshape(1, -1))[0]
        # coordinate_cur_gt = all_gt_boxes_coordinate[ gt_index_cur ]

        ratio_overlap = caculate_2d_overlap( np_all_boxes[np_index_cur,1:6], coordinate_cur, all_gt_boxes[gt_index_cur][:,1:6], all_gt_boxes_coordinate[ gt_index_cur ])
...
if __name__ == '__main__':
    ...
    for i in range(num_box):
        all_pred_boxes.append( np.random.rand( np.random.randint(2,50) ,6))
        all_gt_boxes.append( np.random.rand( np.random.randint(2,50) ,6))
    evaluation_2d(all_pred_boxes,all_gt_boxes)
